...
Its whole content is a distance, so what matters is not what the radio reported
but why a third party should believe that number was measured rather than
chosen.

## The layers

`modules/ultrawidelock_uwb/include/fira_session.h` defines four, numbered with a gap.
...
Layer 2 is what counts against a distance-reduction attack: a spoofed early
first path cannot reproduce the scrambled sequence, so its STS quality
collapses. Layers 1 and 4 are statistical filters: they raise the cost of an
attack without making it cryptographically impossible.
...
anyway), and was declared only in the Zephyr Kconfig tree. On ESP32 it was
**unreachable**, not merely off, so `idf.py` could not produce it and the `#if`
in `ccc_shim_rx.c` could never be true.
...
integrity check, with no field in which to say so.
...
2. **The window matches layer 4.** The verdict is "the last K blocks all
   ...
   blocks.
3. **Presence fails closed; the lock does not.** A mis-tuned floor that refuses
   to open a door locks a human out of their house; one that refuses to sign an
   assertion costs one retry. Presence therefore requires a good STS regardless
   of the build flag; the door keeps shadow behaviour unless
   `CONFIG_ULTRAWIDELOCK_RANGE_GATE_STRICT` is set, now selectable on ESP32 too.
   ...
   **The DWM3001CDK now ships with that flag set** (`apps/dwm3001cdk-lock/prj.conf`),
   ...
   the only board that does. The threshold is still the untuned default below, so
   a real floor remains the release gate. First hardware evidence that the gate
...
**The STS-quality floor is 0**, meaning "defer to the driver verdict". It is now
a build option, `CONFIG_ULTRAWIDELOCK_STS_QUALITY_MIN`, feeding
`FIRA_STS_QUALITY_MIN` (`modules/ultrawidelock_uwb/include/fira_session.h`), so
it can be raised without touching source, but it has never been sized from real
captures, which the Kconfig help has asked for since before this change. The
flight recorder logs `stsq_val` and `stsq_ret` (`flight_recorder.h`), so the
capture path exists: record a walk-up, histogram the quality index, pick a floor
above the noise. Until then, layer 2 rejects only what the DW3000 driver itself
calls bad. That matters more now that the DWM3001CDK enforces rather than
shadows: an untuned floor that is enforced is a door that can refuse to open.

**Relay resistance is not measured.** The time-of-flight argument is structural
and depends on enforcing STS, but no one has attacked this stack. Do not
describe it as tested. Read the literature on distance-reduction attacks against
HRP UWB with STS first.

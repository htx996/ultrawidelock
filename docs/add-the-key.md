...
| It sat on "Adding to Home" and then timed out | Two causes, both fixed in current images. SRP registration failure on old ones: the current image keeps removal objects alive until OpenThread acknowledges them and retries duplicate names. And, before `f7663d4f`, the node timed out on its own durability boundary, because the fabric store worker took a lock its caller already held. Capture the RTT log before restarting a border router |
...
| Renaming the accessory does not stick, or the name is refused | The lock answers `UpdateFabricLabel` and keeps the label in the fabric record, so a rename survives a power cycle. A name another fabric already holds is refused as `LabelConflict`: pick a different one, or rename the other controller first |
...
| The tile works but no walk-up ever unlocks | Either the credential never landed or ranging is not running. Check Wallet first, then the radio: [troubleshooting.md](troubleshooting.md). On an image before `f7663d4f`, a third cause: stored ACL entries were parsed with field ids one below the spec's, so no entry ever granted anything |
...
| An upgrade past `d7ec0269` lost Matter and Home Key | Expected once. The fabric record grew a label field, and a record whose length does not match is dropped at load rather than half-read. Remove the old controller records and pair again |

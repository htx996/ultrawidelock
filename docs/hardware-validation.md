...
| CDK-39 | With a commissioned lock, run CDK-30 for real in Chrome on macOS | The delta goes over Web Bluetooth and the page reports the target SHA-256 | **identify half PASSED 2026-08-27**: Chrome connected, discovered, identified the board and correctly refused, because no delta was published from what it runs. **The upload half is still open** -- no browser has yet driven this board to a finished update, over any transport. A local delta `344da6f022364f7f -> 8613358327a47e00` (7.8 kB) is staged for it via `make fota IMAGE_VERSION=0.3.1 && make ota-local IMAGE_VERSION=0.3.1` |
...
than absolute. CDK-38 is the negative control. CDK-37 was measured with bleak, which shares
CoreBluetooth with Chrome but is not Chrome -- so the row that settled it was
CDK-39's identify half, where the page itself completed discovery and named the
hash on the board. What remains untested is the upload, not the transport.

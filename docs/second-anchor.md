...
RSSI geometry sensitivity all exist because the dongles cannot range; an anchor
that can removes the whole class.
...
stock-iPhone ranging is explicitly unproven in this tree"). Both are void: an
nRF5340 DK with a DWM3000EVB shield is on the bench (a second DWM3001CDK
follows later), and the multi-anchor question was half proven on 2026-07-17.
...
(`examples/zephyr/anchor/src/main.c`). Everything below develops on the DK and
lands on the CDK by changing `ANCHOR_BOARD`.
...
measures `peer_mm` or carries it to the lock. That gap is this plan.
...
Grounds, all already in this tree (protocol-research.md §7):
...
2. **A responder can join from the air alone.** Pre-poll recovery: `mUPSK1`
   comes off the URSK and is static for the session, so a responder holding the
   session keys decrypts the first pre-poll cold, reads `Poll_STS_Index`,
   derives `URSK_KT → dURSK/dUDSK`, and is armed one slot later. No BLE time
   sync needed, which lets a satellite that never spoke BLE participate.
3. **The keys are per-session.** The lock hands the satellite the session's
   ...
   identifies no credential, and opens no lock. A stolen satellite can range
   one live session it was already trusted with; it cannot track the phone
   across sessions or mint an unlock.
4. **A lying satellite cannot open the door.** `ultrawidelock_fusion_may_predict`
   ...
   single-anchor behaviour. The stage-D latch feed is the one place a forged
   OUTSIDE could add capability, and it inherits the witness rules (sealed
   reports; the latch's dwell/session gates stay in front).
...
(stage B'), which needs no new timebase and no unwatched phone behaviour.
...
reference run, so `tol_mm` = 62 (3 sigma), `deadband_mm` = 41 (2 sigma), bench
values pending the install geometry. The -40 mm bias is the tape's reference
point, not the radio's, and is not fitted: fusion consumes the difference of
two links and the constant cancels.
...
The riskiest unknown: the phone reserves the slot, but a real `Response_1` has
never been transmitted, so whether the phone accepts it and emits its timestamp
...
1. Satellite firmware: a new app at the CRED tier, the responder round engine
   ...
   `apps/nrf5340dk-lock`) minus BLE credential auth, plus key injection. A lock
   told the keys instead of negotiating them.
...
- **The satellite joins from air alone.** Given only the lock's session keys,
  ...
  Final with `cper=0` at `stsq` ~60, and decoded `Final_Data`. The URSK ->
  mURSK -> dURSK/dUDSK ladder runs on a board that never touched BLE, so ground
  2 stops being an inference. Receipt: `FINALDATA-2RESP blk=…` plus a `cper=0`
  FINAL on the satellite's console.
...
own defect, kept as the record of how a confident negative got built on a silent
buffer drop.
...
responders was discarded by the size gate before the decode ran. A frame we
delete is indistinguishable from one the initiator never sent, which is the
inference stage B made. The record selection fix (by tag rather than array
position) did NOT unblock this: with two in-order records, position indexing
would have worked. It earns its place where the initiator validates only
responder 1.
...
`CONFIG_ULTRAWIDELOCK_PRETTY_SHELL` still blinds the lock to its own drops. That
wants a `LOG_WRN` before this ships.

## The 04:30 run, kept as the record

Answered no, and **only the observation was settled, not its cause.** The phone
committed to the `N_Resp = 2` round rather than silently falling back: its Final
decoded at POLL+3 with `cper=0` under N=2 derivation, its `Final_Data` at
POLL+4, so both its key schedule and its slot layout used
`Number_Responder_Nodes = 2`. It still emitted `nresp=1` in every `Final_Data`,
confirmed twice, close-range RF ruled out separately. CCC v4 obliges an
initiator to report every responder whose response it VALIDATED and gives it no
field in which to declare itself single-responder, so `nresp=1` is spec-legal
only if validation failed; the round arithmetic checks out against CCC v4
Table 20-2 (Response_l at POLL+1+l / STS+1+l, Final at POLL+N+1, `Final_Data` at
POLL+N+2), so the layout was not the defect.
...
index-1 offset path and one STS derive, so a systematic defect there fails
identically on both, and the probe cannot separate "the phone ignores slot 3"
from "our slot-3 frame fails the phone's validation". Calling it a phone
limitation was a step too far.
...
not conclusively, since both boards run the same `ccc_kdf.c` and a shared derive
...
session the phone already grants on. Lock and satellite share the session keys
(proven, B above) and alternate who transmits `Response_0` by ranging-block
parity (both nodes track block indices, also proven). The phone sees the session
it has always seen; each node gets a true authenticated DS-TWR distance to the
...
responder device is the normative occupant of.
...
1. **RESOLVED: the trust layer tolerates the silent blocks.** `g_range_trust`
   ...
   a block-index gap. A silent block produces no latch, so it cannot clear the
   run, only halve the rate. At K=3 and a 192 ms block, trust arrives about
...
(`aliro-1.0.txt` l.9571-9582) defines a mechanism for exactly this question, and
it is not the multi-responder route:
...
is a real coupling B' does not have, and the thing to design first.

Unproven, and why this is a stage rather than a decision: nothing in this tree
implements MAC Mode or a second round, and the phone's mandatory support is a
spec statement, not a bench measurement. It deserves the one-evening probe
stage B got.

### Constraints the tree imposes on any alternating scheme

From reading the round engine and the fusion layer, 2026-08-21. They apply to
B'; the first applies to B'' too, in weakened form.
...
1. **Alternation breaks the one contract `ultrawidelock_fusion_eval` states.**
   ...
   round." B' guarantees they never are. The anchors are mounted across the
   door plane, so the approach axis IS the baseline axis, `adiff` tends to
   ...
   parity ordering inflates `adiff`, and `ultrawidelock_fusion.c:66` makes
   `may_predict` return false on `!geometry_ok`. It fails CLOSED, predictive
   unlock suppressed on approach rather than an unsafe grant, and the bias flips
   sign every block, so the verdict chatters rather than settling.
   ...
   against whatever report is stored, bounded only by `stale_ms`. B' makes that
...
with no extra protocol. Lock calls `ultrawidelock_satellite_report()` (first
...
The handoff replaced a chain of RTT print → host script → shell paste, so a
walk-up needed a debugger on both boards and the URSK crossed in the clear on a
debug transport. `CONFIG_ULTRAWIDELOCK_SATELLITE_HANDOFF_LOG` still prints that
line and is still bench-only; nothing depends on it now.
...
ms, so it may systematically cost the first blocks, or this may be ordinary
variation between two walk-ups of different lengths. One sample cannot tell
those apart. Measure it before optimising it.
...
outside" is not. The pass criterion above still asks for that, and it needs a
...
DIAGNOSTIC LESSON, including a wrong answer that stood for hours.
...
`CONFIG_ULTRAWIDELOCK_ANCHOR_ROLE` into every report. The port needed three
files and one real bug.
...
RAM was the risk and is not the binding one: the CDK Thread satellite has
23,040 B free, against a lock on the same part shipping at 84.70%. The obvious
reclamation is `CONFIG_MBEDTLS_HEAP_SIZE=16384` in `prj.conf`: the lock measured
a 228 B peak on its own PSA path and now runs 1024. NOT TRIMMED HERE, because
this board's PSA workload is the CCC key ladder rather than the lock's, and only
a heap peak read off hardware settles it. Measure at bring-up, then cut; the
baseline above shows the cut is free.

#### The one-satellite assumption that was really there

The audit of the lock's ingest found two faults, one a security fault. Both are
fixed; neither changes anything for a lock with one satellite.
...
has been on hardware, not a trade worth making with 9,816 B free.
...
release, the same ~12 KB either way, and +2,880 B of RAM in both. The release
profile is worth ~24 KB of flash and 7,296 B of RAM in both rows.
...
that credits the feature with a saving unrelated to it. "Does it fit" is
answered in a column, not a diagonal, and it does fit, with 27,912 B of flash
spare at release.

`RELEASE=1` is not free, it is on-board debugging given up:
`overlay-release.conf` cuts the RTT up-buffer from 8,376 B to 1,024 (nearly all
of its RAM saving), sets `INIT_STACKS=n`, and drops every wrn/inf/dbg format
string. Its own header says not to flash it to a board being brought up, and
stage E is bring-up, so the bench checklist runs on the DEBUG default, which
every size figure here was measured against. `RELEASE=1` is a decision for after
the boards work, not a way to make the feature fit.

**Why 12,772 B, when the geometry is about 1 KB?** `ANCHOR_LINK` cannot be built
on its own yet, and the chain that forces the rest is all in Kconfig:
...
the flash is. Not bundled here: a refactor of a security-sensitive file wants
its own change.
...
2. **First light on the UART, radio off.** `make sat-term`, confirm the shell
   ...
   protocol failure from radio coexistence, and why the plain build exists.
...
number a bench would measure is left blank.
...
exists; the baseline is its output, not a prerequisite.
...
station on whatever channel its AP chose; the satellite associates with nothing
and would sit on channel 1 forever. If those differ, no frame arrives, the lock
hears no reports, the verdict stays UNKNOWN, and UNKNOWN permits, so the door
behaves like a single-anchor lock. Silent.
...
heartbeat re-probes every 15 s, and three misses in a row restart the hunt: an
AP that reboots onto a new channel heals in about a minute.

Two load-bearing details: the lock alone answers, or two satellites would find
...
After the channel reply the lock sends a carrier-marked freshness beacon with
its own random nonce. The satellite restores the shared challenge marker before
...
the current lock-owned nonce, and a satellite whose boot id changes waits for
the next beacon before its restarted counter is believed. Replaying an old
channel probe therefore cannot roll the report receiver back to an old boot and
counter window.
...
outright on `rcfg[12]`. Nothing logs an error, because from each board's point
of view the other never spoke.
...
2. **Wi-Fi and ESP-NOW coexistence on the lock.** The lock rides Matter's radio
   ...
   than never: a report that misses its block pairs with nothing and reads as a
   triangle rejection.
...
and the satellite could drop OpenThread entirely. Latency is better, not worse:
a 7.5-30 ms connection interval against a multicast that must land inside the
...
time on the lock's single core, the constraint that forces the per-frame trace
off before ranging starts. The lock already runs BLE and Thread concurrently
under MPSL, so a second BLE connection is more existing load, not a new class.
Whether it costs measurable arm margin is a MEASUREMENT, not a guess; the same
question about OpenThread cost nothing.
...
transport-independent; only what carries the bytes changes.
...
- Stage B's acceptance question is CLOSED, POSITIVE (2026-08-21 06:25): a stock
  iPhone does report a second responder. B', B'' and TDoA are all retired by it.
- The live risk is stage C, and it is pairing rather than acceptance. Two
  anchors only mean something together if their distances describe the SAME
  ...
  widening the match when reports arrive late must be refused: that is the
  arithmetic that killed B'.
...
- RESOLVED 2026-08-21: the differential error is a CONSTANT, so the two-anchor
  ...
  What remains is a ~20% TAIL, a categorically easier problem: what median
  filtering and the K-consecutive trust layer already exist for.

...
```sh
make bootstrap              # once per machine · the NCS workspace
make dfu-key                # once per clone   · this checkout's MCUboot key
make build
```
...
1. Flash the current image. An upgrade from v0.3 changes the Matter settings
   schema and flash layout, so remove the old accessory from controllers and pair
   it again; the clean break also resets the credential reader identity. Moving to
   a build carrying the fabric label field costs one further re-pair, because the
   fabric record grew and earlier records fail the loader's length check.
...
`HA=1` belongs to the nRF5340 generated data-model variant. DWM3001CDK
multi-admin is standard Matter.
...
- Live Apple Home plus Home Assistant commissioning and fault injection are rows
...
`RELEASE=1`, `SMP=1`, differing only in the multi-admin changes.
...
Reproduce the right-hand column:
...
- `keys/`, the gitignored local signing key from `make dfu-key`.

> [!WARNING]
> Regenerating the signing key is free only until a bootloader is flashed. After
> that MCUboot on that board trusts one key: losing it means the board takes no
> further image over DFU and needs its bootloader replaced over SWD. Being
> gitignored, the key lives in exactly one working directory, so pruning a
> worktree or moving a checkout can destroy it. **Back it up outside this tree.**

...
/*
 * A Matter certificate is TLV, but its signature is the X.509 signature over
 * the DER-encoded TBSCertificate -- the TLV is a re-encoding of an X.509 cert,
 * not a thing signed in its own right. So verifying one means rebuilding that
 * DER and hashing THAT.
 *
 * This node used to hash the TLV span instead, which cannot verify any real
 * certificate and never did: the responder does not walk chains at all (it
 * takes IPK possession as proof of fabric membership), and the client's walk
 * had only ever been fed certificates our own test encoder signed the same
 * wrong way, so it agreed with itself. Measured 2026-08-22 against a live
 * Home Assistant fabric: every TLV span fails, including on an Apple NOC whose
 * sessions were up on the board at the time.
 *
 * The conversion is mechanical and narrow on purpose -- it accepts the Matter
 * operational profile and nothing else, and refuses anything it does not

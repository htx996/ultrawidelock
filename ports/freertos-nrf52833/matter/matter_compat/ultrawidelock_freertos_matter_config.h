...
/*
 * THE PASSCODE IS 58490394, AND IT IS NOT CHIP'S 20202021.
 *
 * This said the opposite until a commissioning attempt failed on it. The values
 * below were CHIP's published test pairing once; apps/dwm3001cdk-lock/Kconfig
 * records replacing them with this project's own, generated by
 * scripts/spake2p_verifier.py, so that knowing the SDK is not enough to
 * commission the lock. The verifier was carried across to this file and the
 * comment was not.
 *
 * Nothing on the device can catch that. The passcode is deliberately absent
 * from the image -- the augmented SPAKE2+ form exists so the device holds only
 * the verifier -- so a wrong passcode in a COMMENT costs a bench session and no
 * build error. What it looks like on hardware: BTP connects, PBKDFParamResponse
 * and Pake2 both go out, and the commissioner hangs up the moment it checks cB.
 * Every layer below Matter reads as healthy, because it is.
 *
 * Derive it, do not trust this. PBKDF2-HMAC-SHA256 over the passcode as four
 * little-endian bytes, with the salt and iteration count below, gives 80 bytes:
 * w0 = first 40 mod n, w1 = last 40 mod n, and the verifier is w0 (32 B) then
 * the uncompressed w1*G (65 B). 58490394 reproduces the constant below exactly;
 * 20202021 does not.
 *
 * The manual pairing code for this passcode and this discriminator is
 * 36505035696. The oracle's build re-derives and prints it and refuses on
 * disagreement; this build does neither yet.
 */
